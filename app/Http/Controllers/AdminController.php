...
class AdminController extends Controller
{
    ...
    public function manage_exam()
    {
        ...
        $data['exams']=Oex_exam_master::orderBy('id','DESC')->select(['tbl_exam_masters.*','tbl_categories.name as cat_name'])->join('tbl_categories','tbl_exam_masters.category','=','tbl_categories.id')->get()->toArray();
    ...
    {
        $validator = Validator::make($request->all(),[
            'title'=>'required|unique:tbl_exam_masters,title',
            'exam_date'=>'required',
    ...
    public function manage_students(){
        ...
        $data['students'] = user_exam::select(['tbl_user_exams.*','users.name','tbl_exam_masters.title as ex_name','tbl_exam_masters.exam_date'])
    ...
    {
        $std = user_exam::where('id',$id)->first();
        $std->delete();
    ...
    public function edit_students_final(Request $request){
        ...
        $std->update();
        echo json_encode(array('status'=>'true','message'=>'Updated successfully','reload'=>url('admin/manage_students')));
    }

    //Registered student page
    public function registered_students()
    {
        $data['users'] = User::orderBy('id','DESC')->get()->all();
        return view('admin.registered_students',$data);
    }

    //Deleting students registered
    ...
    {
        $data['questions']=Oex_question_master::orderBy('id','DESC')->where('exam_id',$id)->get()->toArray();
    ...
    {       
        ...
        $averageYesAnsForUser = Oex_result::where('exam_id',$std_exam->exam_id)->avg('yes_ans');
        $averageNoAnsForUser = Oex_result::where('exam_id',$std_exam->exam_id)->avg('no_ans');

        $data['averageYesAnsForUser'] = $averageYesAnsForUser;
        $data['averageNoAnsForUser'] = $averageNoAnsForUser;
        // $data['combinedAverageForUser'] = ($averageYesAnsForUser + $averageNoAnsForUser) / 2;
